...
type
  AudioFormat = cint
  AudioSpec* = object
    freq*: cint ## DSP frequency (samples per second); see Remarks for details
    format*: AudioFormat ## audio data format; see Remarks for details
    channels*: uint8 ## number of separate sound channels: see Remarks for details
    silence*: uint8 ## audio buffer silence value (calculated)
    samples*: uint16 ## audio buffer size in samples (power of 2); see Remarks for details
    size*: uint32 ## audio buffer size in bytes (calculated)
    callback*: pointer ## the function to call when the audio device needs more data; see Remarks for
    userdata*: pointer ## a pointer that is passed to callback (otherwise ignored by SDL)

proc openAudioDevice*(device: cstring, iscapture: cint, spec, obtained: ptr AudioSpec, allowChanges: cint): cint {.importc: "SDL_OpenAudioDevice".}

# proc pauseAudioDevice*(dev: cint): void {.importc: "SDL_PauseAudioDevice".}
# proc resumeAudioDevice*(dev: cint): void {.importc: "SDL_PauseAudioDevice".}
proc pauseAudioDevice*(dev: cint, pause: cint): void {.importc: "SDL_PauseAudioDevice".}
